...
contract MetaTxForwarder is EIP712, Ownable, ReentrancyGuard {
    ...
    /// @param durationSeconds La duración del bucket en segundos (0 = sin expiración)
    /// @param useCustom true para usar config personalizada, false para usar la config por defecto
    ...
    function _executeCreate(Forward calldata f, bytes calldata data) internal {
        ...
        // Obtener la configuración del bucket para este deployer
        ...
        unchecked {
            ...
            // Verificar límite del bucket si está configurado
            if (limit != 0 && dur != 0) {
