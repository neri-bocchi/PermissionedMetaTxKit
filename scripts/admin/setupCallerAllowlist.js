...
//  const relayerAddress = owner.address; // In this example, using the same wallet
  ...
  try {
    ...
    const isAllowed = await hub.isCallerAllowed(relayerAddress);
    ...
    const tx = await hub.setCallerAllowed(relayerAddress, true);
    ...
    const isNowAllowed = await hub.isCallerAllowed(relayerAddress);
